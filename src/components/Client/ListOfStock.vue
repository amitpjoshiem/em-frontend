<template>
  <div class="p-5">
    <SwdSubHeader title="List of Stock Certificates or Bonds" />
    <div v-if="!isLoading">
      <div class="mb-5">
        <el-checkbox
      ...
      <div v-if="!state.availabilityDocuments" class="min-h-[175px] mb-5 p-5 border border-input-border rounded-md">
        <SwdUpload
          :upload-data="{ collection: 'list_of_stock_certificates_or_bonds' }"
...
export default {
  ...
  setup() {
    ...
    const collection = 'list_of_stock_certificates_or_bonds'
    ...
    const saveStep = async () => {
      ...
      const res = await updateSteps({ list_of_stock_certificates_or_bonds: statusStep.value })
      if (!('error' in res)) {
        useAlert({
          title: 'Success',
          type: 'success',
    ...
    const changeStatus = async () => {
      ...
      await updateSteps({ list_of_stock_certificates_or_bonds: status })
    }
