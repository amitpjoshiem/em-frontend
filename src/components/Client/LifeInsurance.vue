<template>
  <div class="p-5">
    <SwdSubHeader title="Life Insurance" />
    <div v-if="!isLoading">
      <div class="mb-5">
        <el-checkbox
      ...
      <div v-if="!state.availabilityDocuments" class="min-h-[175px] mb-5 p-5 border border-input-border rounded-md">
        <SwdUpload
          :upload-data="{ collection: 'life_insurance_annuity_and_long_terms_care_policies' }"
...
export default {
  ...
  setup() {
    ...
    const collection = 'life_insurance_annuity_and_long_terms_care_policies'
    ...
    const saveStep = async () => {
      ...
      const res = await updateSteps({ life_insurance_annuity_and_long_terms_care_policies: statusStep.value })
      if (!('error' in res)) {
        useAlert({
          title: 'Success',
          type: 'success',
    ...
    const changeStatus = async () => {
      ...
      await updateSteps({ life_insurance_annuity_and_long_terms_care_policies: status })
    }
