<template>
  <div class="p-5">
    <SwdSubHeader title="Social Security Information" />
    <div v-if="!isLoading">
      <div class="mb-5">
        <el-checkbox
      ...
      <div v-if="!state.availabilityDocuments" class="min-h-[175px] mb-5 p-5 border border-input-border rounded-md">
        <SwdUpload
          :upload-data="{ collection: 'social_security_information' }"
...
export default {
  ...
  setup() {
    ...
    const collection = 'social_security_information'
    ...
    const saveStep = async () => {
      ...
      const res = await updateSteps({ social_security_information: statusStep.value })
      if (!('error' in res)) {
        useAlert({
          title: 'Success',
          type: 'success',
    ...
    const changeStatus = async () => {
      ...
      await updateSteps({ social_security_information: status })
    }
