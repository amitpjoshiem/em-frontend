  ...
  setup() {
    ...
    const saveStep = async () => {
      if (!isDisabledSaveBtn.value) {
        const res = await updateSteps({ ['property_casualty']: store.state.globalComponents.uploadPropertyDocsStatus })
        if (!('error' in res)) {
    ...
    const isDisabledSaveBtn = computed(() => {
      return isLoadingUpdateSteps.value || store.state.globalComponents.uploadPropertyDocsStatus === null
    })
