<template>
  <aside class="bg-primary min-h-screen flex-col w-[68px] sm:flex hidden">
    ...
    <div
      v-if="isAuth && ($can('advisor', 'all') || $can('super-admin', 'all'))"
      class="flex flex-col items-center flex-grow w-[68px] fixed top-1/3"
    >
      <div class="text-red-500">SAS</div>
      <router-link
        :to="{ name: 'sa-dashboard' }"
        class="item flex justify-center items-center cursor-pointer w-full h-14"
        :class="{ active: getRouteName === 'sa-dashboard' }"
      >
        <InlineSvg v-if="getRouteName === 'sa-dashboard'" :src="IconDashboardActive" />
      ...
      <router-link
        :to="{ name: 'sa-activity' }"
        class="item flex justify-center items-center cursor-pointer w-full h-14"
        :class="{ active: getRouteName === 'sa-activity' }"
      >
        <InlineSvg v-if="getRouteName === 'sa-activity'" :src="IconAssetsActive" />
        <InlineSvg v-else :src="IconAssets" />
      </router-link>
      <router-link
        :to="{ name: 'sa-pipeline' }"
        class="item flex justify-center items-center cursor-pointer w-full h-14"
        :class="{ active: getRouteName === 'sa-pipeline' }"
      >
        <InlineSvg v-if="getRouteName === 'sa-pipeline'" :src="IconActivityPipeLine" />
      ...
      >
        <InlineSvg
          v-if="getRouteName === 'all-list' || getRouteName === 'opportunities-list' || getRouteName === 'clients-list'"
          :src="IconLeadsActive"
        />
